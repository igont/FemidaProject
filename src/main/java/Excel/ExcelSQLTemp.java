...
public class ExcelSQLTemp // Функции для связи SQL и EXCEL
{
	public static String EXCEL_FILE_NAME = "Рейтинг судей по Абсолютно реальному бою.xlsx";
	...
	public static void connect()
	{
		try
		{
			...
			connection = DriverManager.getConnection((BD_URL + Config.databaseName), Config.user, Config.userPass);
	...
	public static void execute(String str) throws SQLException // Отправляем запрос, который ничего не возвращает
	{
		statement.execute(str);
	}
	public static void addRefereeAccount(RefereeAccount refereeAccount) throws SQLException // Добавляем нового судью
	{
		PreparedStatement preparedStatement = connection.prepareStatement("insert into "+ Config.refereeTableName +"(f_name, s_name, m_name, city, phone, calc_points, category, birth_year, club_type, club_name) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
	...
	public static List<RefereeAccount> readParticipants() throws IOException, SQLException // Читаем все данные о судьях в файле
	{
		...
		InputStream fis = classloader.getResourceAsStream(EXCEL_FILE_NAME); // Читаем файл
	...
	public static void reCreateTableReferee() throws SQLException
	{
		String drop = "drop table "+ Config.refereeTableName;
		String createNew = "create table "+ Config.refereeTableName +" (" +
				"id smallserial, " +
				"f_name text, " +
				"s_name text, " +
				"m_name text, " +
		...
		try
		{
			...
			System.out.println("Drop table: "+ Config.refereeTableName);
		}
		catch(PSQLException e)
		{
			System.out.println("Drop table Failed: "+ Config.refereeTableName);
		}
		ExcelSQLTemp.execute(createNew);
		System.out.println("Create table: "+ Config.refereeTableName);
	}
